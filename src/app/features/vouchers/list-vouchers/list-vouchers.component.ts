...
export class ListVouchersComponent {
  ...
  vouchers = computed(() => this.orderService
    .order()
    ?.payments
  ...
  constructor() {
    this.deleteVoucher$.pipe(
      takeUntilDestroyed(),
      switchMap(payment => {
        const service = this.services.find(service => service.adapterId === payment.adapterId);
        if (!service) {
          this.toastService.error(`Voucher service for ${payment.adapterId} is not available`);
          return EMPTY;
        }
