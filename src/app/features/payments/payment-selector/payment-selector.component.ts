...
@Component({
  ...
  imports: [SelectButton, FormsModule, Card],
...
export class PaymentSelectorComponent {
  ...
  constructor() {
    connect(this.state)
      .with(() => {
        ...
        const enabledPayments = configs
          .filter((config) => {
            ...
            const hasPaymentService = this.paymentServices.some(
              (service) => service.adapterId === config.id,
            );
            if (isPayment && !hasPaymentService) {
              this.toastService.warn(
                `Payment service for ${config.id} is not available`,
              );
            }
      ...
      .with(() => {
        const state = untracked(() => this.state());
        const payment = this.orderService
          .order()
        ...
        switchMap((adapterId) => {
          ...
          if (!enabledPayments.includes(adapterId)) {
            this.toastService.error(
              `Payment service for ${adapterId} is not available`,
            );
            return EMPTY;
          }

          const currentPayment = this.currentPayment();
          const currentPaymentService = this.paymentServices.find(
            (service) => service.adapterId === currentPayment?.adapterId,
          );
          const nextPaymentService = this.paymentServices.find(
            (service) => service.adapterId === adapterId,
          )!;
          if (currentPaymentService) {
            return currentPaymentService
              .removePayment(currentPayment!.id!)
              .pipe(switchMap(() => nextPaymentService.createPayment()));
          } else {
