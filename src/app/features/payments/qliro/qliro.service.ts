...
import { DataService } from '~/app/features/payments/qliro/data.service';
...
export class QliroService implements IPaymentService {
  private adapters = inject(ADAPTERS);
  readonly adapterId = this.adapters.payment.Qliro;
  ...
  createPayment(): Observable<QliroCheckoutOrder> {
    return this.dataService.createPayment(this.orderId()).pipe(
      retry(2),
      catchError(() => {
        this.toastService.error('Failed to create qliro payment');
        return EMPTY;
      }),
    );
  }

  getPayment(paymentId: string): Observable<QliroCheckoutOrder> {
    return this.dataService.getPayment(this.orderId(), paymentId).pipe(
      retry(2),
      catchError(() => {
        this.toastService.error('Failed to fetch qliro payment');
        return EMPTY;
      }),
    );
  }
  ...
  removePayment(paymentId: string): Observable<void> {
    return this.dataService.removePayment(this.orderId(), paymentId).pipe(
      retry(2),
      catchError(() => {
        this.toastService.error('Failed to remove qliro payment');
        return EMPTY;
      }),
    );
  }
  ...
  updateShippingOption(
    ...
    return this.dataService
      .updateShippingOption(this.contextService.context()!, paymentId, payload)
      .pipe(
        retry(2),
        catchError(() => {
