...
export class KustomComponent {
  ...
  constructor() {
    ...
    this.redirectInitiated$.pipe(
      switchMap(() => this.kustomService.getPayment(this.paymentId()!))
    ).subscribe(() => {
  ...
  private addEventListeners() {
    ...
    klarnaApi((api: KlarnaApi) => {
      api.on({
        ...
        [KlarnaEvent.ShippingOptionChange]: () => this.shippingOptionChanged$.next(),
        [KlarnaEvent.RedirectInitiated]: () => this.redirectInitiated$.next(),
        [KlarnaEvent.LoadConfirmation]: () => this.handleLoadConfirmation.bind(this),
        [KlarnaEvent.CanNotCompleteOrder]: () => this.handleCanNotCompleteOrder.bind(this),
        [KlarnaEvent.NetworkError]: () => this.handleNetworkError.bind(this),
        [KlarnaEvent.ValidationCallback]: this.handleValidationCallback.bind(this),
      });
    });
  }

  private removeEventListeners() {
    // Klarna API doesn't have a remove method, the handlers will be garbage collected
  }
