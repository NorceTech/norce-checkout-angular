...
import {
  ...
  IngridEventName,
  WindowIngrid,
} from '~/app/features/shippings/ingrid/ingrid.types';
...
export class IngridComponent {
  ...
  private shippingId = computed(() => {
    return this.orderService
      .order()
      .shippings?.filter((s) => s.state !== 'removed')
      ...
      if (this.useAddressForm()) {
        api.on(IngridEventName.SummaryChanged, (data, meta) => {
          if (meta?.total_value_changed) {
            this.ingridService
              .updateShipping(this.shippingId()!)
              .pipe(finalize(() => this.syncService.triggerRefresh()))
              .subscribe();
          }
          if (
            meta?.delivery_address_changed ||
            meta?.delivery_address_changed
          ) {
            this.ingridService
              .updateCustomer(this.shippingId()!)
              .pipe(finalize(() => this.syncService.triggerRefresh()))
              .subscribe();
          }
        });
