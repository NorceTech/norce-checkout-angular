...
export class ShippingSelectorComponent {
  ...
  constructor() {
    connect(this.state)
      .with(() => {
        ...
        const enabledShippings = configs
          .filter(config => {
            ...
            const hasService = this.shippingService.some(service => service.adapterId === config.id)
      ...
      .with(() => {
        const state = untracked(() => this.state());
        const shipping = this.orderService.order()
          ?.shippings
    ...
    this.useShippingAdapter$.pipe(
      switchMap(adapterId => {
        const enabledShippings = this.state().enabledShippings;
        if (!enabledShippings) return EMPTY;
        if (!enabledShippings.includes(adapterId)) {
          this.toastService.error(`Shipping service for ${adapterId} is not available`);
          return EMPTY;
        }

        const currentShipping = this.currentShipping();
        const currentShippingService = this.shippingService.find(service => service.adapterId === currentShipping?.adapterId);
        const nextShippingService = this.shippingService.find(service => service.adapterId === adapterId)!;
        if (currentShippingService) {
          return currentShippingService.removeShipping(currentShipping!.id!).pipe(
