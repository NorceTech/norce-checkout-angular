...
import {CreateOrderCommand, PlatformService} from '~/app/core/platform/platform.service';
...
export class NorceAdapterService extends PlatformService {
  ...
  createOrder(command: CreateOrderCommand): Observable<Context> {
    return this.dataService.createOrder(
      command.merchant,
      command.channel,
      Number(command.cartReference),
      command.culture
    ).pipe(
      map((response) => new Context({
      ...
      catchError((error: HttpErrorResponse) => {
        ...
        this.toastService.error(errorResponse?.message ?? 'Failed to create order');
        return EMPTY;
      }),
    );
  }
